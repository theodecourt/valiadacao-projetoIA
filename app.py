...
def search_for_product(product):
    ...
    return (product.lower(), [])
        ...
        # Constrói biblioteca em paralelo
def get_products_search_library(unique_products):
    lib = {}
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        futures = {executor.submit(search_for_product, p): p for p in unique_products}
...
def run_flow_for_product(key, resultado, retries=3):
    ...
    return ["NOT_FOUND"]
        ...
        def escolhe_produtos(resultado):
    out = {}
    for k, res in resultado.items():
        out[k] = run_flow_for_product(k, res)
        time.sleep(1)
    return out
        ...
        for res in df["resultado_search"]:
            ids.append(escolhe_produtos(res))
            time.sleep(2)
        df["id_agente_escolhe_produtos"] = ids
...
def conta_sim_nao(obj):
    ...
    return pd.Series({'quantidade_sim': sim, 'quantidade_nao': nao})
        ...
        df['perc_acerto'] = df.apply(lambda r: round((r['quantidade_sim'] / (r['quantidade_sim'] + r['quantidade_nao']) * 100), 2) if (r['quantidade_sim'] + r['quantidade_nao']) > 0 else 0, axis=1)
        ...
        df_excel = df.rename(columns={'nome_fornecedor_agente_escolhe_produtos': 'produtos_fornecedores'})[['produtos_fornecedores', 'output_agente_verificacao_limpo', 'quantidade_sim', 'quantidade_nao', 'perc_acerto']]
        ...
        st.download_button("📥 Baixar planilha de resultados", data=output, file_name="resultados.xlsx")
